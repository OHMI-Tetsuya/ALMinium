...
#  ${GEM} install -f strscan --no-document || fatal_error_exit ${BASH_SOURCE}
...
# MariaDB設定
#CHK=`grep "character-set-server=utf8mb4" /etc/my.cnf`
#if [ "${CHK}" = "" ]; then
#sudo mv /etc/my.cnf /etc/my.cnf.org
#  sudo bash -c "cat /etc/my.cnf.org | sed -e \"s/\[mysqld_safe\]/character-set-server=utf8mb4\n\n\[mysqld_safe\]/g\" > /etc/my.cnf"
